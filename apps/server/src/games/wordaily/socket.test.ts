...
import { createWordailyServer } from './index.js'
import { clearRoomsForTest } from './rooms.js'

type TestServer = ReturnType<typeof createWordailyServer> & {
  url: string
}
...
async function createTestServer(): Promise<TestServer> {
  const server = createWordailyServer()
...
async function joinRoom(client: Socket, roomCode: string, playerName = 'Nina') {
  const playerState = once<{ id: string; guesses: unknown[] }>(client, 'player:state')
...
describe('socket room flow', () => {
  ...
  it('re-attaches a reconnecting player during room sync', async () => {
    ...
    const privateState = once<{ id: string; guesses: unknown[] }>(reconnectedPlayer, 'player:state')
  ...
  it('allows host-token sync to start and restart a game from a new socket', async () => {
    ...
    await expect(started).resolves.toMatchObject({ endsAt: expect.any(Number) })
