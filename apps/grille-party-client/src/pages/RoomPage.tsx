...
export default function RoomPage() {
  ...
  const [longestWord, setLongestWord] = useState<{ word: string; path: import('../types').CellPos[] } | null>(null)
  ...
  useEffect(() => {
    ...
    socket.on('game:ended', ({ results: r, longestPossibleWord, longestPossiblePath }: { results: PlayerResult[]; longestPossibleWord?: string; longestPossiblePath?: import('../types').CellPos[] }) => {
      setResults(r)
      if (longestPossibleWord && longestPossiblePath) {
        setLongestWord({ word: longestPossibleWord, path: longestPossiblePath })
      }
    })
  ...
  if (isPlayerView) {
    if (phase === 'results' && results) {
      return <ResultsCinematic results={results} grid={room?.grid} longestWord={longestWord} onDone={handleRestart} hideReplay={!isHost} roomCode={isHost ? roomCode : undefined} />
    }
  ...
  return (
    <div className="game-screen flex flex-col lg:h-screen lg:overflow-hidden">
      ...
      {phase === 'results' && results && (
        <ResultsCinematic results={results} grid={room?.grid} longestWord={longestWord} onDone={handleRestart} roomCode={roomCode} />
      )}
    </div>
  )
}
